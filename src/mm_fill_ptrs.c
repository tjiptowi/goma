	      ...
	      const Exo_DB *exo)
{
  ...
  for (i = 0; i < MAX_ELEMENT_INDICES_RELATED; i++) {
    if (ei->linkedEIelems[i] == relatedElem) {
      slot = i;
      alreadyDone = 1;
      break;
    }
  ...
  if (slot == -1) 
	...
	exit(-1);
      ...
      ei->linkedEIelems[i] = relatedElem;
    }
  /*
   *  We check this condition here. But, I think it should already be
   *  redundant.
   */
  if (ei->owningElementForColVar[v] != relatedElem)
    {
      fprintf(stderr, "inconsistent entry in ei->owningElementForColVar[%d], %d, %d\n",
	      v, ei->owningElementForColVar[v], relatedElem);
      EH(-1, "linkedParent_ei() error\n");
    }


  /*
   *  Link the current element's ei->owningElement_ei_ptr[v] to a slot
   ...
   */
  ei->owningElement_ei_ptr[v] = eiRelated[slot];
  ...
  if (!alreadyDone) 
    {
      load_ei(relatedElem, exo, ei->owningElement_ei_ptr[v]);

      pd = pd_glob[mn];
      Current_EB_ptr = Element_Blocks + ei->elem_blk_index;
...
int
load_ei(const int elem, const Exo_DB *exo, struct Element_Indices *ei_ptr_fill)
      ...
      ***********************************************************************/
{
  ...
  int iLvdesc; /* Running index for the number of local variable 
		...
		*/
  struct Element_Indices *ei_ptr = ei;
		      ...
		      for (ln = 0; ln < num_local_nodes_Parent; ln++) 
			{
			  ...
			  if (ei_ptr->owningElementForColVar[v] == -1 || 
			      ...
			      if (enunks) 
				{
				  ...
				  if (mode == 0) 
				    {
				      linkParent_ei(elem, elem_Parent, v, exo); 
				      pd	 = pd_glob[mn];    
				    }
				} 
			    } 
 ...
 *
 *		ei->dof[TEMPERATURE] = 4
 *		ei->dof[PRESSURE]    = 3    ( in 2D with piecewise linear)
 *
 *	and...
 *		ei->dof_list[TEMPERATURE] = { 0, 1, 2, 3}
 *		ei->dof_list[PRESSURE]    = { 8, 8, 8}
 *
 *	If none, then...
 *		ei->dof[SOLUTE]	     = 0;
 *		ei->dof_list[SOLUTE] = NULL;
 *
					  ...
					  * computationally expensive pointers */
{
  ...
  for (i = 0; i < nnodes; i++) {
    gnn = Proc_Elem_Connect[ei->iconnect_ptr + i];
 ...
 eqn = R_SHELL_NORMAL3;
  ...
  if (efv->TALE) {
    for (k = 0; k < exo->num_dim; k++) {
      dofs = ei->dof[MESH_DISPLACEMENT1 + k];
      for (i = 0; i < dofs; i++) {
	ie = Proc_Elem_Connect[ei->iconnect_ptr + i];
	evp->initial_displacements[k][i] =
	    efv->init_displacement_ndl_val[k] + ie;
      }
      dofs = ei->dof[SOLID_DISPLACEMENT1+k];
      for (i = 0; i < dofs; i++) {
	ie = Proc_Elem_Connect[ei->iconnect_ptr + i];
     ...
     ******************************************************************/
{
  ...
  for (i = 0; i < ei->Num_Lvdesc; i++) {
    vd = ei->Lvdesc_vd_ptr[i];
    printf("     ");
    printf("%-5d", i);
    pvn(ei->Lvdesc_to_Var_Type[i], vd);
    printf("  %-5d", ei->Lvdesc_to_MatID[i]);
    printf("  %-5d", ei->Lvdesc_Numdof[i]);
    printf("\n");
  }
  printf("     "); fprint_line(stdout,"-", 60); printf("\n\n");
  ...
  for (i = 0; i < ei->Num_Lvdesc; i++) {
    ...
    pvn(ei->Lvdesc_to_Var_Type[i], vd);
    printf(", MatID = %-5d", ei->Lvdesc_to_MatID[i]);
    printf("\n");
    printf("        ");
    printf("i_vdesc  lnn  lvdof   ledof   gnn   gun  active owned\n");
    printf("        "); fprint_line(stdout,"-", 60);
    for (j = 0; j < ei->Lvdesc_Numdof[i]; j++) {
      printf("        ");   
      printf("  %-5d", j);
      printf("  %-5d ", ei->Lvdesc_to_Lnn[i][j]);
      printf("  %-5d ", ei->Lvdesc_to_lvdof[i][j]);
      ledof = ei->Lvdesc_to_ledof[i][j];
      printf(" %-5d ", ledof);
      printf("%-6d", ei->Lvdesc_to_Gnn[i][j]);
      printf("%-6d ", ei->Lvdesc_to_Gun[i][j]);
      printf("%-6d ", ei->active_interp_ledof[ledof]);
      printf("%-6d ", ei->owned_ledof[ledof]);
      if (ei->ieqn_ledof[ledof] != ei->Lvdesc_to_Gun[i][j]) {
	printf("WARNING ieqn_edof = %5d", ei->ieqn_ledof[ledof]);
      }
      printf("\n");
    }
    printf("        "); fprint_line(stdout,"-", 60);
    ...
    for (ln = 0; ln < nnodes; ln++) {
      printf("        ");   
      printf(" %-5d", ln);
      printf("  %-5d", ei->Lvdesc_Lnn_Numdof[i][ln]);
      printf("  %-5d", ei->Lvdesc_Lnn_to_lvdof[i][ln]);
      printf("    %-5d", ei->Lvdesc_Lnn_to_Offset[i][ln]);
      printf("\n");
    }
    printf("        "); fprint_line(stdout,"-", 60);
  }
  ...
  for (i = 0; i <  MAX_VARIABLE_TYPES; i++) {
    if (ei->dof[i] > 0) {
      ...
      printf("     %-5d", i);
      printf("    %-5d", ei->dof[i]);
      printf("    %-5d", ei->Num_Lvdesc_Per_Var_Type[i]);
      printf("\n");
    }
  }
  printf("     "); fprint_line(stdout,"-", 60); printf("\n\n");
  ...
  for (i = 0; i < MAX_VARIABLE_TYPES; i++) {
	...
	printf(" note-> only subvar=0 dofs are included below");
      ...
      for (j = 0; j < ei->dof[i]; j++) {
	printf("        ");   
	printf("  %-5d", j);
	ln =  ei->dof_list[i][j];
	numidofs[ln]++;
	printf("%-5d ", ln);
	ledof = ei->lvdof_to_ledof[i][j];
	printf("%-5d ", ledof);
	printf(" %-5d ", ei->lvdof_to_lvdesc[i][j]);
	printf("  %-5d ", ei->lvdof_to_lvdesc_dof[i][j]);
	printf("     %-5d ", ei->Baby_Dolphin[i][j]);
	printf(" %-6d", ei->gnn_list[i][j]);
	printf(" %-6d", ei->gun_list[i][j]);
	printf(" %-5d", ei->lvdof_to_row_lvdof[i][j]);
	printf(" %-5d", ei->active_interp_ledof[ledof]);
	printf("\n");
      }
      printf("        "); fprint_line(stdout,"-", 70);
	...
	printf("  %-5d ", ei->ln_to_dof[i][ln]);
	printf("  %-5d ", ei->ln_to_first_dof[i][ln]);
	printf("\n");
      }
      printf("        "); fprint_line(stdout,"-", 60);
    }
